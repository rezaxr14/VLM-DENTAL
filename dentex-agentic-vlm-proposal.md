...
**Aim 2 — System.** Design and implement an agentic VLM pipeline: a reasoning core (Qwen2.5-VL) that iteratively calls a tool suite (zoom/crop, contrast enhancement, segmentation/grounding, FDI numbering) and outputs a structured quadrant–enumeration–diagnosis report.
...
1. Zero-shot GPT-4o / Qwen2.5-VL, no tools, single pass.
...
| 3. SFT (3B) | 3–4 | Kaggle/Colab free tier (QLoRA, 16GB) | Wire tools into the VLM's tool-calling interface; run Stage 1 SFT on Qwen2.5-VL-3B; validate format/tool-call reliability |
...
- Backbone: Qwen2.5-VL-7B-Instruct (fallback 3B) — see §5.3.
