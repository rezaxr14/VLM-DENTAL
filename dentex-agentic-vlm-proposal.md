...
1. **Generation**: for each labeled abnormal tooth in DENTEX training data, run a locally-hosted **Qwen/Qwen3.5-9B** (served via vLLM inside the Kaggle/Colab session — not a frontier API, and not the laptop) as the **trace generator**, orchestrated as a real multi-turn tool-calling agent in LangGraph. Generation stays *conditioned on the known ground-truth label* and seeded toward ground-truth regions of interest — we keep this deliberately, since blind exploration on an untuned model would tank the yield of usable, correctly-labeled training data.
2. **Real tool execution**: the agent's tool calls (crop coordinates seeded from the ground-truth bounding box) execute for real through the LangGraph tool loop — `zoom_crop`, `window_level`, `denoise`, `contralateral_compare` genuinely run against the source image, and the real resulting image is appended to the trajectory at that turn. This replaces the earlier pre-computed-image-plus-`<fake_tool_call>`-tag approach: the model is still told where to look and what the answer is, but it now actually receives the pixels it claims to have inspected, so its stated visual evidence is checkable against real tool output rather than only plausible-sounding.
...
**Reasoning core (decided): Qwen/Qwen3.5-9B, a single backbone used across every compute tier** — as the trace-generation teacher (§5.2) *and* the trained/deployed reasoning policy, collapsing the earlier staged plan now that 4-bit QLoRA and Unsloth's optimized GRPO/GSPO path fit Qwen3.5-9B comfortably on a single free Colab/Kaggle T4, removing the need for a smaller prototyping-only fallback. This is not an arbitrary pick: Qwen3.5-9B provides native multimodality, strong medical VQA reasoning, and built-in `<think>...</think>` tags for structured CoT, keeping results directly comparable to the strongest existing precedents in both medical RL-VLM work and general RL+tool-use-VLM work. It has a working, documented GRPO training recipe (Hugging Face TRL's VLM-GRPO cookbook, Unsloth-accelerated) to build from rather than starting from scratch. Output format is constrained to a `<think>...</think>`/tool-call block/`<answer>...</answer>` schema so format and accuracy rewards are computable automatically.
...
3. **Grounding tool** — a YOLOv8m detector, trained with 5-fold cross-validation given DENTEX's small size (simplified from the originally-proposed Grounding-DINO/SAM segmentation model to plain bounding-box detection, which is sufficient for the zoom/crop targeting this tool needs to support) fine-tuned on DENTEX's quadrant-enumeration boxes, returning a box for a requested tooth or region. Validation has reached mAP50 ≈ 0.647 (R ≈ 0.90, P ≈ 0.588) — clearing the val mAP50 > 0.5 bar set for wiring it into the live agent loop, so `locate_tooth` is no longer held back. The precision/recall balance (catches ~90% of teeth, ~41% of predicted boxes are false positives) is a reasonable fit for a grounding *tool* rather than a final answer — an occasional wasted zoom into a healthy region costs a turn, but a missed tooth costs a finding. For context against DENTEX's own Enumeration subtask — the closest published comparison point, since it's the same "which tooth, what FDI position" task rather than Quadrant or Diagnosis — the grounding tool's 5-fold mean (mAP50 0.641 ± 0.008, mAP50-95 0.376 ± 0.007; best fold mAP50 0.647, mAP50-95 0.386) numerically exceeds the 2023 challenge's top three teams on their held-out test set (AR/AP50/AP: 1st 0.682/0.511/0.353, 2nd 0.648/0.501/0.335, 3rd 0.699/0.466/0.308) and the official HierarchicalDet baseline (0.550/0.446/0.287). This is not a like-for-like SOTA claim, though: those are blind, containerized-submission scores on a held-out test set, while ours are validation-fold scores on data drawn from the same training pool — a materially easier bar to clear. The honest framing for the paper is "clears the quality bar for live agent use," not "beats DENTEX SOTA," unless a genuinely held-out slice or an actual grand-challenge submission is obtained later.
