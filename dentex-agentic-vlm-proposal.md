...
1. **Generation**: for each labeled abnormal tooth in DENTEX training data, run a locally-hosted **Qwen3-VL-8B-Thinking** (served via vLLM inside the Kaggle/Colab session — not a frontier API, and not the laptop) as the **trace generator**, orchestrated as a real multi-turn tool-calling agent in LangGraph. Generation stays *conditioned on the known ground-truth label* and seeded toward ground-truth regions of interest — we keep this deliberately, since blind exploration on an untuned 8B model would tank the yield of usable, correctly-labeled training data.
...
**Reasoning core (decided): Qwen3-VL-8B-Thinking, a single backbone used across every compute tier** — as the trace-generation teacher (§5.2) *and* the trained/deployed reasoning policy, collapsing the earlier staged 2B-then-8B plan now that Unsloth's optimized GRPO/GSPO path fits Qwen3-VL-8B comfortably on a single free Colab/Kaggle T4, removing the need for a smaller prototyping-only fallback. This is not an arbitrary pick: it is the exact backbone family used by Med-R1, MedVLM-R1, VTool-R1, and OpenMedReason, which keeps results directly comparable to the strongest existing precedents in both medical RL-VLM work and general RL+tool-use-VLM work, and it already has a working, documented GRPO training recipe (Hugging Face TRL's VLM-GRPO cookbook, Unsloth-accelerated) to build from rather than starting from scratch. The **Thinking** variant specifically, since it's tuned to break problems into steps and give evidence-based answers — a better match for the step-by-step diagnostic reasoning this project trains than the plain Instruct release. Output format constrained to a `<think>...</think>`/tool-call block/`<answer>...</answer>` schema so format and accuracy rewards are computable automatically.
...
| 3. SFT (8B) | 3–4 | Kaggle/Colab free tier (QLoRA, 16GB) | Wire tools into the VLM's tool-calling interface; run Stage 1 SFT on Qwen3-VL-8B-Thinking; validate format/tool-call reliability |
| 4. GRPO smoke test (8B) | 4–5 | Kaggle/Colab free tier, reduced group size | Get the GRPO loop running end-to-end at small scale (G≈4) to validate reward computation and rollout mechanics before spending scarcer 4090 time |
| 5. First real GRPO run, full group size | 5–7 | Occasional RTX 4090 (24GB), this semester | Full-group (G≈8) GRPO on the same Qwen3-VL-8B-Thinking backbone — this step scales group size and throughput, not model size |
| 6. Full-scale training & reward sweep | 7–9 | Dedicated RTX 4090 / server, next semester | Full 8B GRPO training; complete reward-weight sweep (§5.5); move to EasyR1 only if 4090 throughput becomes the bottleneck |
...
| Compute cost of GRPO | Single 8B backbone throughout, Unsloth-accelerated LoRA/QLoRA fine-tuning, modest group size (G ≈ 4–8 depending on tier), capped tool-call budget per rollout |
...
- Backbone: Qwen3-VL-8B-Thinking, single tier throughout, no smaller fallback needed — see §5.3.
...
- Which model to use as the independent verifier in §5.2 — generation is now decided (locally-hosted Qwen3-VL-8B-Thinking, see §5.3); the verifier just needs to be a different model family than the generator, with budget planned for k-sample self-consistency generation across ~1,000 training cases.
