...
- Grounding Tool (`locate_tooth`) uses YOLOv8m (5-fold cross-validation) and is **live** in the agent loop. Ensure `GROUNDING_MODEL_PATH` is set in your `.env`. Multi-dataset validation mAP50 ≈ 0.8695 (DENTEX+Tufts) — see `docs/TRACE_GEN_CONFIG.md` and `docs/YOLO_CV_RESULTS.md`.
